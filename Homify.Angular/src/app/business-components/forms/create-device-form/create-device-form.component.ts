...
export class CreateDeviceFormComponent {
  ...
  handleSubmit() {
	...
	createDevice(device: string){
		const { name, model, description, ppalPicture, isExterior, isInterior, photos } = this.form.value;
		switch (device) {
			case 'camera':
			...
			case 'movement-sensor':
				...
				this.DeviceService.createMovementSensor(movementSensorRequest).subscribe(
					response => {
